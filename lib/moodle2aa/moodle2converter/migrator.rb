module Moodle2AA::Moodle2Converter
  class Migrator
    ...
    def initialize(source_file, output_dir)
      ...
      @extractor = Moodle2AA::Moodle2::Extractor.new(source_file)
      @output_dir = output_dir
    end

    def migrate()
      @extractor.extract do |moodle_course|
        Moodle2AA::MigrationReport.create(@output_dir, moodle_course);
        ...
        cc_course.pages += convert_generic_activities(moodle_course) if Moodle2AA::MigrationReport.convert_unknown_activities?

        cc_course.pages.each do |canvas_page|
          canvas_page.href = generate_unique_resource_path(Moodle2AA::CanvasCC::Models::Page::WIKI_CONTENT, canvas_page.title)
        end
    ...
    def convert_assessments(quizzes, choices, feedbacks, questionnaires)
      assessment_converter = Moodle2AA::Moodle2Converter::AssessmentConverter.new
      assessments = []
    ...
    def convert_question_banks(question_categories)
      bank_converter = Moodle2AA::Moodle2Converter::QuestionBankConverter.new
    ...
    def convert_html!(cc_course, moodle_course)
      ...
      cc_course.assessments.each do |assessment|
        ...
        assessment.items.each do |item|
          if item.is_a?(Moodle2AA::CanvasCC::Models::QuestionGroup)
            item.questions.each do |question|
              convert_question_html!(question, html_converter)
            end
          elsif item.is_a?(Moodle2AA::CanvasCC::Models::Question)
    ...
    def convert_question_html!(question, html_converter)
      ...
      question.post_process! if question.is_a?(Moodle2AA::CanvasCC::Models::CalculatedQuestion)
    end
