# NOTE: Generates a datatable script for a moodle calculated question type.
# Wiris questions have their datatable script generated by the WirisConverter
class Moodle2AA::Learnosity::DataTableEngineScriptGenerator
  ...
  def format_body
    (
      ...
      @answers.map { |a| "  #{a[:name]} = #{format_formula(a[:formula])};" }
    ).join("\n")
  end

  def format_formula(expr)
    # moodle treats log as ln
    expr.gsub(/log\(/, 'ln(')
  end
end
