    ...
    def write_settings(assignment_dir, assignment)
      xml = Nokogiri::XML::Builder.new(encoding: 'UTF-8') do |xml|
        xml.assignment('identifier' => assignment.assignment_resource.identifier,
          ...
          xml.due_at Moodle2AA::CC::CCHelper.ims_datetime(assignment.due_at) if assignment.due_at
          xml.lock_at Moodle2AA::CC::CCHelper.ims_datetime(assignment.lock_at) if assignment.lock_at
          xml.unlock_at Moodle2AA::CC::CCHelper.ims_datetime(assignment.unlock_at) if assignment.unlock_at
          xml.all_day_date Moodle2AA::CC::CCHelper.ims_datetime(assignment.all_day_date) if assignment.all_day_date
          xml.peer_reviews_due_at Moodle2AA::CC::CCHelper.ims_datetime(assignment.peer_reviews_due_at) if assignment.peer_reviews_due_at
      ...
      File.open(File.join(assignment_dir, Moodle2AA::CanvasCC::Models::Assignment::ASSIGNMENT_SETTINGS_FILE), 'w') { |f| f.write(xml) }
    end
