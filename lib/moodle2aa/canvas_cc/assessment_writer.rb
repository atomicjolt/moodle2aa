    ...
    def write_assessment_meta_xml(assessment)
      xml = Nokogiri::XML::Builder.new(encoding: 'UTF-8') do |xml|
        xml.quiz(
          ...
          Moodle2AA::CanvasCC::Models::Assessment::META_ATTRIBUTES.each do |attr|
            quiz_node.send(attr, assessment.send(attr)) unless assessment.send(attr).nil?
          end
          Moodle2AA::CanvasCC::Models::Assessment::DATETIME_ATTRIBUTES.each do |attr|
            quiz_node.send(attr, Moodle2AA::CC::CCHelper.ims_datetime(assessment.send(attr))) unless assessment.send(attr).nil?
          end
        end
      end.to_xml
            ...
            assessment_node.section(:ident => 'root_section') do |section_node|
              assessment.items.each do |item|
                ...
                when Moodle2AA::CanvasCC::Models::Question
                  Moodle2AA::CanvasCC::QuestionWriter.write_question(section_node, item)
                when Moodle2AA::CanvasCC::Models::QuestionGroup
                  Moodle2AA::CanvasCC::QuestionGroupWriter.write_question_group(section_node, item)
                end
              end
            end
          end
