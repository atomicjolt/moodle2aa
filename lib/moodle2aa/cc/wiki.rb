    ...
    def initialize(mod)
      ...
      @pages = mod.pages.map do |page|
        if page.version == page_versions[page.page_name]
          ...
          Moodle2AA::OpenStruct.new(:title => page.page_name, :body => body, :href => href, :identifier => create_key(href))
        end
      end.compact
      ...
      if @pages.empty?
        ...
        href = @href_template % slug
        @pages = [Moodle2AA::OpenStruct.new(:title => @title, :body => mod.summary, :href => href, :identifier => create_key(href))]
      end
