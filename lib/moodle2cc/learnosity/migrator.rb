module Moodle2CC::Learnosity
  class Migrator
    ...
    def migrate(moodle_course)
      ...
      if Moodle2CC::MigrationReport.group_by_quiz_page?
      ...
      html_converter = Moodle2CC::Learnosity::Converters::HtmlConverter.new(learnosity.files, moodle_course)
      ...
      @path = Moodle2CC::Learnosity::Writers::AtomicAssessments.new(learnosity, moodle_course).create(@output_dir) if Moodle2CC::MigrationReport.generate_archive?
    end

    private

    def create_learnosity_export(moodle_course)
      learnosity = Moodle2CC::Learnosity::Models::Export.new
      learnosity
    end

    def create_learnosity_export_meta(moodle_course)
      meta = Moodle2CC::Learnosity::Models::ExportMeta.new
    ...
    def group_by_quiz_page(moodle_course)
      ...
      moodle_course.quizzes.each do |quiz|
        ...
        quizcat = Moodle2CC::Moodle2::Models::Quizzes::QuestionCategory.new
    ...
    def consolidate_shared_questions(questions, moodle_quiz)
      new_question = Moodle2CC::Learnosity::Models::Moodle2::CalculatedQuestionGroup.new
    ...
    def consolidate_quiz_page(questions, moodle_quiz, page)
      new_question = Moodle2CC::Learnosity::Models::Moodle2::QuizPageGroup.new
    ...
    def convert_questions(question_categories, moodle_course, html_converter)
      Moodle2CC::Learnosity::Converters::QuestionBankConverter.new(moodle_course, html_converter).convert(question_categories)
    end
    ...
    def convert_assignments(quizzes, moodle_course)
      ...
      quizzes.each do |quiz|
        ...
        assignments << [Moodle2CC::Learnosity::Converters::AssignmentConverter.new(moodle_course).convert(quiz, moodle_course.question_categories)]
      end
      assignments
    end
    ...
    def convert_files(moodle_course)
      ...
      moodle_course.files.each do |file|
        files << Moodle2CC::Learnosity::Converters::FileConverter.new(moodle_course).convert(file)
      end
      files
    end
