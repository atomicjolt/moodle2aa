module Moodle2CC
  class MigrationReport
    ...
    def self.create(out_dir, moodle_course)
      Thread.current[:__moodle2cc_migration_report__] = MigrationReport.new(out_dir, moodle_course)
    end

    def self.close()
      Thread.current[:__moodle2cc_migration_report__].csvfile.close if Thread.current[:__moodle2cc_migration_report__].csvfile
      Thread.current[:__moodle2cc_migration_report__] = nil
    end

    def self.instance()
      Thread.current[:__moodle2cc_migration_report__]
    end
